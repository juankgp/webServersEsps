...
//#ifdef MYSSID
  if(caso){
    ...
    WiFi.mode(WIFI_AP_STA);
      ...
      for (int i = 0; i < 4; i++)
      {
        WiFi.begin(ssidtxt.c_str(), passtext.c_str());
...
/////////////
 ...
 setInterval(function() {
  ...
  server.on("/tiempo1", HTTP_GET, [](AsyncWebServerRequest *request){
    String cadena = String(vHora) + "h: " + String(vMin) +"m: " + String(vSeg) + "s" ;
  request->send_P(200,"text/plain",cadena.c_str());
});
  server.on("/timeuc", HTTP_GET, [](AsyncWebServerRequest *request){
    String cadena = String(iHora) + "h: " + String(iMin) +"m: " + String(iSeg) + "s" + "<br>T evacuación:" + String(itEspera);
  request->send_P(200,"text/plain",cadena.c_str());
});
server.on("/countdown", HTTP_GET, [](AsyncWebServerRequest *request){
    String cadena =  String(vtEspera);
  request->send_P(200,"text/plain",cadena.c_str());
});
...
server.on("/estadoest", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    String cadena =  "<img src=" + estado + "> <br>" + texto;
...
server.on("/wifidata", HTTP_GET, [](AsyncWebServerRequest *request){
    ...
    if (request->hasParam(PARAM_INPUT_2)) {
     ...
     // Serial.println("Llego value");
      ...
      String wifiData1 = String(inputMessage);
    ...
    String passtext = wifiData.substring(sepssid+1,seppass);
     ...
     Serial.println("Datos Wifi: " + wifiData1);
    
     writetxt(wifiData1);
     delay(5000);
     ESP.restart();http://192.168.4.1/config
    }
    else {
      inputMessage = "No message sent";
